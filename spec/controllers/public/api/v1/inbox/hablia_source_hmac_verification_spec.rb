...
RSpec.describe 'Public Inbox API source signature', type: :request do
  ...
  describe 'with hmac_mandatory' do
    context 'with a valid signature for the source_id' do
      ...
      it 'returns the existing contact_inbox on contacts#create' do
        post "#{inbox_path}/contacts", params: { source_id: contact_inbox.source_id }, headers: owner_headers

        expect(response).to have_http_status(:success)
        expect(response.parsed_body['source_id']).to eq(contact_inbox.source_id)
        expect(contact_inbox.reload.hmac_verified).to be(false)
      end
    end
    ...
    context 'with the signature of another source_id' do
      let!(:attacker_contact_inbox) { create(:contact_inbox, inbox: api_channel.inbox, source_id: '+525550000002') }
      ...
      it 'does not let a query param stand in for the signed source_id' do
