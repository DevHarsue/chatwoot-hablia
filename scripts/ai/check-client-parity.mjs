...
function usage(message) {
  ...
  console.error(
    'Uso: node scripts/ai/check-client-parity.mjs [--repo <ruta>] [--format text|json]'
  );
  process.exit(2);
}
...
if (
  repoIndex !== -1 &&
  (!args[repoIndex + 1] || args.filter(arg => arg === '--repo').length !== 1)
) {
  usage('--repo requiere una única ruta');
}
if (
  formatIndex !== -1 &&
  (!['text', 'json'].includes(args[formatIndex + 1]) ||
    args.filter(arg => arg === '--format').length !== 1)
) {
...
if (args.some((arg, index) => !knownArgumentIndexes.has(index)))
  usage('Argumento no reconocido');

const root =
  repoIndex === -1
    ? workspaceRoot
    : path.resolve(workspaceRoot, args[repoIndex + 1]);
...
function section(text, name, indentation = 0) {
  ...
  const start = lines.findIndex(line =>
    new RegExp(`^ {${indentation}}${escaped(name)}:\\s*$`).test(line)
  );
  if (start === -1) return '';
  const next = lines
    .slice(start + 1)
    .findIndex(
      line =>
        ...
        !line.startsWith(' '.repeat(indentation + 1))
    );
...
function list(text, name, indentation = 0) {
  const inline = text.match(
    new RegExp(`^ {${indentation}}${escaped(name)}:\\s*\\[\\]\\s*$`, 'm')
  );
  if (inline) return [];
...
function mapping(text, name, indentation = 0) {
  if (
    new RegExp(`^ {${indentation}}${escaped(name)}:\\s*\\{\\}\\s*$`, 'm').test(
      text
    )
  )
    return true;
  ...
  const directEntries = entries.filter(
    line => line.match(/^ */)[0].length === indentation + 2
  );
  return (
    directEntries.length > 0 &&
    directEntries.every(line =>
      new RegExp(`^ {${indentation + 2}}[A-Za-z0-9_-]+:`).test(line)
    )
  );
}
...
function validateYamlShape(content, file) {
  ...
  for (const [index, rawLine] of lines.entries()) {
    ...
    if (indentation % 2 !== 0) {
      fail(
        'INVALID_YAML',
        `${file}:${index + 1}: la indentación debe usar múltiplos de dos`
      );
      continue;
    }
    const contentLine = rawLine.slice(indentation);
    const sequence = contentLine.match(/^-\s+(.+)$/);
    const mappingMatch = contentLine.match(/^([A-Za-z0-9_-]+):(?:\s*(.*))?$/);
    if (!sequence && !mappingMatch) {
      fail(
        'INVALID_YAML',
        `${file}:${index + 1}: sintaxis YAML fuera del subconjunto permitido`
      );
      continue;
    }
    const kind = sequence ? 'sequence' : 'mapping';
    const value = sequence ? sequence[1] : mappingMatch[2] || '';
    ...
    if (
      (doubleQuoted && !/^"(?:[^"\\]|\\.)*"$/.test(scalarValue)) ||
      (singleQuoted && !/^'(?:[^']|'')*'$/.test(scalarValue))
    ) {
      fail(
        'INVALID_YAML',
        `${file}:${index + 1}: string entre comillas inválida o sin cierre`
      );
      continue;
    }
    if (!doubleQuoted && !singleQuoted && /:\s/.test(scalarValue)) {
      fail(
        'INVALID_YAML',
        `${file}:${index + 1}: los escalares con dos puntos deben estar entre comillas`
      );
      continue;
    }
    if (
      (scalarValue.startsWith('[') && scalarValue !== '[]') ||
      (scalarValue.startsWith('{') && scalarValue !== '{}')
    ) {
      fail(
        'INVALID_YAML',
        `${file}:${index + 1}: colecciones inline no están permitidas por este contrato`
      );
      continue;
    }

    while (stack.length > 1 && stack.at(-1).indentation >= indentation)
      stack.pop();
    const parent = stack.at(-1);
    if (indentation !== parent.indentation + 2) {
      fail(
        'INVALID_YAML',
        `${file}:${index + 1}: la estructura salta un nivel de indentación`
      );
      continue;
    }
    if (!parent.canHaveChildren) {
      fail(
        'INVALID_YAML',
        `${file}:${index + 1}: un valor escalar no puede contener hijos`
      );
      continue;
    }
    if (parent.childKind && parent.childKind !== kind) {
      fail(
        'INVALID_YAML',
        `${file}:${index + 1}: no se pueden mezclar listas y mapas en el mismo nivel`
      );
      continue;
    }
    ...
    if (mappingMatch) {
      parent.keys ||= new Set();
      if (parent.keys.has(mappingMatch[1])) {
        fail(
          'INVALID_YAML',
          `${file}:${index + 1}: clave duplicada (${mappingMatch[1]})`
        );
        continue;
      }
      parent.keys.add(mappingMatch[1]);
    }
    const node = {
      indentation,
      childKind: null,
...
function descriptorClientPath(client, file) {
  if (client === 'claude') {
    return (
      ...
      /^plugins\/[^/]+\/\.claude-plugin\//.test(file)
    );
  }
  return (
    file.startsWith('.codex/') || /^plugins\/[^/]+\/\.codex-plugin\//.test(file)
  );
}
...
function parseDescriptor(content, descriptorPath) {
  ...
  if (parsed.schemaVersion !== '1')
    fail('DESCRIPTOR_SCHEMA', `${descriptorPath}: schema_version debe ser 1`);
  if (!parsed.type || !parsed.name)
    fail(
      'DESCRIPTOR_RESOURCE',
      `${descriptorPath}: resource.type y resource.name son obligatorios`
    );
  if (!['claude', 'codex'].includes(parsed.client))
    fail(
      'DESCRIPTOR_CLIENT',
      `${descriptorPath}: client debe ser claude o codex`
    );
  if (!parsed.source)
    fail('DESCRIPTOR_SOURCE', `${descriptorPath}: source es obligatorio`);
  if (!Array.isArray(parsed.outputs) || !parsed.outputs.length)
    fail(
      'DESCRIPTOR_OUTPUTS',
      `${descriptorPath}: outputs no puede estar vacío`
    );
  if (!Array.isArray(parsed.transformations))
    fail(
      'DESCRIPTOR_CONTRACT',
      `${descriptorPath}: transformations es obligatorio (puede ser [])`
    );
  if (!parsed.hasCapabilityMapping)
    fail(
      'DESCRIPTOR_CONTRACT',
      `${descriptorPath}: capability_mapping es obligatorio (puede ser {})`
    );
  if (!parsed.validationCommands.length)
    fail(
      'DESCRIPTOR_CONTRACT',
      `${descriptorPath}: validation.commands no puede estar vacío`
    );
  if (!parsed.owner)
    fail('DESCRIPTOR_OWNER', `${descriptorPath}: owner es obligatorio`);
  if (!['direct', 'sha256', 'generated-check'].includes(parsed.strategy)) {
    fail(
      'DESCRIPTOR_DRIFT',
      `${descriptorPath}: drift.strategy debe ser direct, sha256 o generated-check`
    );
  }
  return parsed;
}
...
function parseExceptions(content, exceptionsPath) {
  ...
  for (const line of lines) {
    ...
    if (listItem) {
      ...
      if (current.list === 'approved_by')
        current.approvers.push(unquote(listItem[1]));
      continue;
    }
  }
  if (current) records.push(current);
  ...
  for (const record of records) {
    ...
    if (!/^AIC-EXC-\d{3,}$/.test(record.id || ''))
      fail(
        'EXCEPTION_ID',
        `${exceptionsPath}: id inválido (${record.id || 'vacío'})`
      );
    if (!['claude', 'codex'].includes(record.client))
      fail('EXCEPTION_CLIENT', `${record.id}: client debe ser claude o codex`);
    if (!record.paths.length)
      fail('EXCEPTION_PATH', `${record.id}: paths no puede estar vacío`);
    if (
      !record.reason ||
      !record.owner ||
      ...
      fail(
        'EXCEPTION_METADATA',
        `${record.id}: requiere reason, owner, approved_by, expires_on y validation`
      );
    }
    const datePattern = /^\d{4}-\d{2}-\d{2}$/;
    const expiry = new Date(`${record.expires_on}T23:59:59Z`);
    if (
      !datePattern.test(record.expires_on || '') ||
      Number.isNaN(expiry.valueOf()) ||
      expiry.toISOString().slice(0, 10) !== record.expires_on ||
      expiry < new Date()
    ) {
      fail(
        'EXCEPTION_EXPIRED',
        `${record.id}: excepción vencida o fecha inválida (${record.expires_on || 'vacía'})`
      );
    }
    for (const rawPath of record.paths) {
      try {
        ...
        if (!descriptorClientPath(record.client, exceptionPath)) {
          fail(
            'EXCEPTION_SCOPE',
            `${record.id}: ${exceptionPath} no pertenece al adaptador ${record.client}`
          );
        }
      } catch (error) {
  ...
  for (const exception of exceptions) {
    const commandPaths = exception.paths.filter(value => {
      try {
        ...
        return (
          covers(candidate, '.claude/commands') ||
          candidate.startsWith('.claude/commands/')
        );
      } catch (error) {
    ...
    let workflowMap;
    try {
      workflowMap = normalizeRelative(
        exception.workflow_map,
        `${exception.id}.workflow_map`
      );
      if (
        workflowMap.startsWith('.claude/') ||
        workflowMap.startsWith('.codex/')
      ) {
        fail(
          'EXCEPTION_WORKFLOW_MAP',
          `${exception.id}: workflow_map debe ser portable (${workflowMap})`
        );
        continue;
      }
      if (!(await exists(path.join(root, workflowMap)))) {
        fail(
          'EXCEPTION_WORKFLOW_MAP',
          `${exception.id}: workflow_map inexistente (${workflowMap})`
        );
        continue;
      }
    } catch (error) {
    ...
    for (const rawPath of commandPaths) {
      ...
      if (!/^\.claude\/commands\/[a-z0-9][a-z0-9-]*\.md$/.test(commandPath)) {
        fail(
          ...
          `${exception.id}: los comandos deben declararse como archivos explícitos (${commandPath})`
        );
        continue;
      }
      if (!(await exists(path.join(root, commandPath)))) {
        fail(
          'EXCEPTION_COMMAND_PATH',
          `${exception.id}: comando inexistente (${commandPath})`
        );
        continue;
      }
      ...
      if (!row.test(workflowContent)) {
        fail(
          'EXCEPTION_WORKFLOW',
          `${exception.id}: falta el workflow portable para /${command} en ${workflowMap}`
        );
      }
    }
  }
...
async function validateDescriptors(files, exceptions) {
  ...
  const descriptorFiles = files.filter(file =>
    /^\.agents\/adapters\/[^/]+\/[^/]+\/(claude|codex)\.yaml$/.test(file)
  );
  for (const descriptorPath of descriptorFiles) {
    ...
    if (descriptorPath !== expected)
      fail(
        'DESCRIPTOR_LOCATION',
        `${descriptorPath}: la ubicación no coincide con resource/client`
      );
    try {
      const source = normalizeRelative(
        descriptor.source,
        `${descriptorPath}.source`
      );
      if (
        source.startsWith('.claude/') ||
        source.startsWith('.codex/') ||
        /^plugins\/[^/]+\/\.(claude|codex)-plugin\//.test(source)
      ) {
        fail(
          'CLIENT_AS_SOURCE',
          `${descriptorPath}: source no puede ubicarse bajo una ruta exclusiva de cliente`
        );
      }
      if (!(await exists(path.join(root, source))))
        fail(
          'BROKEN_SOURCE',
          `${descriptorPath}: source inexistente (${source})`
        );
      descriptor.source = source;
      descriptor.outputs = descriptor.outputs.map(value =>
        normalizeRelative(value, `${descriptorPath}.outputs`)
      );
    } catch (error) {
    ...
    for (const output of descriptor.outputs) {
      if (!(await exists(path.join(root, output))))
        fail(
          'BROKEN_OUTPUT',
          `${descriptorPath}: output inexistente (${output})`
        );
    }
      ...
      for (const output of descriptor.outputs) {
        try {
          ...
          if (!sourceContent.equals(outputContent)) {
            fail(
              'DIRECT_DRIFT',
              `${descriptorPath}: source y output difieren; use sha256 o generated-check si hay transformación`
            );
          }
        } catch {
          fail(
            'DIRECT_DRIFT',
            `${descriptorPath}: direct requiere un output de archivo comparable (${output})`
          );
        }
      }
    }
    if (descriptor.strategy === 'sha256') {
      if (
        ...
        !/^[a-f0-9]{64}$/.test(descriptor.outputHash) ||
        descriptor.outputs.length !== 1
      ) {
        fail(
          'SHA256_DRIFT',
          `${descriptorPath}: sha256 requiere source_sha256, output_sha256 y un único output`
        );
      } else {
        const sourceContent = await readFile(
          path.join(root, descriptor.source)
        );
        const outputContent = await readFile(
          path.join(root, descriptor.outputs[0])
        );
        if (sha256(sourceContent) !== descriptor.sourceHash)
          fail('SHA256_DRIFT', `${descriptorPath}: source_sha256 no coincide`);
        if (sha256(outputContent) !== descriptor.outputHash)
          fail('SHA256_DRIFT', `${descriptorPath}: output_sha256 no coincide`);
      }
    }
    if (descriptor.strategy === 'generated-check') {
      ...
      if (!generatedSkill || !descriptor.validationCommands.includes(command)) {
        fail(
          'GENERATED_DRIFT',
          `${descriptorPath}: generated-check solo aplica a adaptadores Claude de skills generados por ${command}`
        );
      } else {
    ...
    for (const exceptionId of descriptor.exceptionIds) {
      if (!exceptions.some(exception => exception.id === exceptionId)) {
        fail(
          'UNKNOWN_EXCEPTION',
          `${descriptorPath}: exception_id desconocido (${exceptionId})`
        );
      }
    }
  }
  ...
  for (const descriptor of descriptors) {
    ...
    if (clients.has(descriptor.client))
      fail(
        'DUPLICATE_DESCRIPTOR',
        `${key}: hay más de un descriptor ${descriptor.client}`
      );
  ...
  for (const [key, clients] of byResource) {
    ...
    const waived =
      only.exceptionIds.length > 0 &&
      only.outputs.every(output =>
    ...
    if (!waived)
      fail(
        'MISSING_CLIENT_ADAPTER',
        `${key}: faltan adaptadores Claude o Codex, sin excepción vigente`
      );
  }

  for (const file of files.filter(candidate =>
    /^\.agents\/skills\/[^/]+\/SKILL\.md$/.test(candidate)
  )) {
  ...
  for (const file of files.filter(candidate =>
    /^\.agents\/agents\/[^/]+\/AGENT\.md$/.test(candidate)
  )) {
    const name = file.split('/')[2];
  ...
  for (const file of files.filter(candidate =>
    /^plugins\/[^/]+\/PLUGIN\.md$/.test(candidate)
  )) {
    ...
    if (!resource) {
      fail(
        'PLUGIN_ADAPTERS',
        `${file}: faltan descriptores Claude/Codex o una excepción AIC`
      );
      continue;
    }
    for (const client of ['claude', 'codex']) {
      ...
      const allowed = descriptor?.exceptionIds.some(id => {
        const exception = exceptions.find(candidate => candidate.id === id);
        return (
          exception?.client === client && exceptionCovers(exception, manifest)
        );
      });
      if (!allowed)
        fail(
          'PLUGIN_MANIFEST',
          `${file}: falta ${manifest} sin excepción AIC vigente`
        );
    }
  }
  for (const command of generatedChecks) {
    const result = spawnSync(
      process.execPath,
      [path.join(root, 'scripts/ai/sync-skill-adapters.mjs'), '--check'],
      {
        cwd: root,
        encoding: 'utf8',
      }
...
async function validateClientCoverage(files, descriptors, exceptions) {
  const candidates = files.filter(
    file =>
      descriptorClientPath('claude', file) ||
      descriptorClientPath('codex', file)
  );
  ...
  for (const file of candidates) {
    ...
    if (
      descriptors.some(
        descriptor =>
          ...
          descriptor.outputs.some(output => covers(output, file))
      )
    )
      continue;
    const exception = exceptions.find(
      candidate =>
        candidate.client === client && exceptionCovers(candidate, file)
    );
    if (exception) {
    ...
    fail(
      ...
      `${file}: recurso exclusivo de ${client} sin descriptor ni excepción vigente`
    );
  }
  for (const exception of exceptions) {
    if (
      !usedExceptions.has(exception.id) &&
      !descriptors.some(descriptor =>
        descriptor.exceptionIds.includes(exception.id)
      )
    ) {
      fail(
        'UNUSED_EXCEPTION',
        `${exception.id}: no cubre ningún recurso ni descriptor`
      );
    }
  }
}
...
async function validateImports(files) {
  ...
  try {
    ...
    fail(
      'MISSING_CLAUDE_ADAPTER',
      `CLAUDE.md no está disponible como archivo regular (${error.message})`
    );
  }
  ...
  for (const file of files.filter(
    candidate => candidate.endsWith('.md') && importable(candidate)
  )) {
    ...
    for (const line of content.replaceAll('\r\n', '\n').split('\n')) {
      ...
      if (!match) continue;
      try {
        const imported = normalizeRelative(
          path.posix.normalize(
            path.posix.join(path.posix.dirname(file), match[1])
          ),
          `${file} import`
        );
        if (!(await exists(path.join(root, imported))))
          fail('BROKEN_IMPORT', `${file}: import inexistente @${match[1]}`);
      } catch (error) {
...
async function validateReviewGuidelines() {
  ...
  if (!(await exists(guidelinesPath))) {
    fail(
      ...
      'Falta REVIEW_GUIDELINES.md para el pre-review portable'
    );
    return;
  }
  ...
  for (const sectionName of requiredSections) {
    if (!content.includes(sectionName)) {
      fail(
        'REVIEW_GUIDELINES_CONTRACT',
        `REVIEW_GUIDELINES.md: falta la sección ${sectionName}`
      );
    }
  }
}
...
function validateToml(file) {
  ...
  const runners =
    process.platform === 'win32'
      ? [
          ['py', ['-3', parser, path.join(root, file)]],
          ['python3', [parser, path.join(root, file)]],
        ]
      : [
          ['python3', [parser, path.join(root, file)]],
          ['py', ['-3', parser, path.join(root, file)]],
  ...
  for (const [command, commandArgs] of runners) {
    ...
    fail(
      'INVALID_TOML',
      `${file}: ${(result.stderr || result.stdout || 'el parser TOML falló').trim()}`
    );
    return;
  }
  fail(
    'TOML_PARSER_UNAVAILABLE',
    `${file}: se requiere Python 3.11+ con tomllib para validar TOML`
  );
}
...
async function validateManifests(files) {
  for (const file of files.filter(
    candidate =>
      candidate === '.mcp.json' ||
      /(^|\/)\.(claude|codex)-plugin\/plugin\.json$/.test(candidate)
  )) {
    try {
      ...
      if (file.endsWith('plugin.json')) {
        ...
        if (
          ...
          fail(
            ...
            `${file}: name debe coincidir con el directorio del plugin`
          );
        }
        if (
          parsed.version &&
          ...
          fail(
            'INVALID_PLUGIN_MANIFEST',
            `${file}: version debe usar semver estricto`
          );
        }
      }
    } catch (error) {
      fail(
        ...
        `${file}: JSON inválido (${error.message})`
      );
    }
  }
    ...
    for (const [index, line] of lines.entries()) {
      for (const [kind, pattern] of patterns) {
        if (pattern.test(line))
          fail(
            'SECRET_DETECTED',
            `${file}:${index + 1}: patrón ${kind} detectado`
          );
      }
    }
  }
...
async function main() {
  ...
  const exceptionsPath = path.join(
    ...
    'docs',
    'devops',
    'ai-client-exceptions.yaml'
  );
  if (!(await exists(exceptionsPath))) {
    fail(
      'MISSING_EXCEPTION_REGISTRY',
      'Falta docs/devops/ai-client-exceptions.yaml'
    );
  }
  ...
  if (outputFormat === 'json') {
    console.log(
      JSON.stringify({ root, valid: errors.length === 0, errors }, null, 2)
    );
    ...
    console.log(
      'Paridad Claude/Codex validada: adaptadores, excepciones, rutas, manifests e indicadores de secretos conformes.'
    );
  }
